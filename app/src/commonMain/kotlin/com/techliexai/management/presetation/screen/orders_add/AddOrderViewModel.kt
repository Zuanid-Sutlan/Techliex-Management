            ...
            viewModelScope.launch {
                try {
                    productRef.snapshots.collect { snapshot ->
                        val products = snapshot.documents.map { it.data<ProductHunt>() }
...
//                val orderId = System.currentTimeMillis().toInt() // Unique ID based on timestamp
            ...
            viewModelScope.launch {
                val id = try { orderRef.get().documents.size + 1 } catch(e: Exception) { 1 }
...
//                    },
                ...
                // 3. Push to Firebase
                try {
                    orderRef.document("Order_$id").set(newOrder)
                    EventManager.hideLoading()
                    EventManager.showMessage("Order placed successfully!", MessageType.SUCCESS)
                    EventManager.navigateBack()
                } catch (e: Exception) {
                    EventManager.hideLoading()
                    EventManager.showMessage("Failed to save order: ${e.message}", MessageType.ERROR)
                }
            }
        }
    }
