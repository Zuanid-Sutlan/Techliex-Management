    ...
    private fun createUser() {
        viewModelScope.launch {
            ...
            try {
                ...
                val id = accountRef.get().documents.size + 1
                try {
                    accountRef.document(_state.value.username).set(User(id = id, name = _state.value.name, username = _state.value.username, password = _state.value.password, role = _state.value.role, isActive = true))
